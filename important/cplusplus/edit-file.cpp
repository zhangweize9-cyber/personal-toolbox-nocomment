...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        std::cout << "--- HSL Mode [W/S:move, Enter:output, Q:quit] ---\n\n";
        ...
        if (c == 'q' || c == 'Q') break; // 识别 q 退出
        if (c == 'w' && cursor > 0) cursor--;
        if (c == 's' && cursor < lines.size() - 1) cursor++;
        if (c == '\n') {
            disableRawMode();
            std::cout << "\n\x1b[38;2;155;89;182;1mSelected:\x1b[0m " << lines[cursor] << "\n";
            return 0;
        }
    }
